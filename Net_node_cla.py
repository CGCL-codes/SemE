...
class Concat_Layer(nn.Module):
    ...
    def __init__(self, size):
        ...
        self.linear = nn.Linear(args.dim * size, args.dim)

    def forward(self, apa_embedding, apcpa_embedding, appa_embedding, structure_embedding):
        embedding = torch.cat((apa_embedding, apcpa_embedding, appa_embedding, structure_embedding), 1)
...
class Net(nn.Module):
    def __init__(self, output_dim0, output_dim1, output_dim2, output_dim):
        ...
        self.hawkes_process_layer3 = Hawkes_process_Layer(output_dim2)
        self.concat_Layer = Concat_Layer(4)
    ...
    def forward(self, semantic_embedding0, interval0, edge_index0,
                ...
                semantic_embedding2, interval2, edge_index2,
                central_node_emb0, central_node_emb1, central_node_emb2, data, structure_embedding):
        embedding0 = self.linear3(semantic_embedding0)
        embedding1 = self.linear4(semantic_embedding1)
        embedding2 = self.linear5(semantic_embedding2)
        ...
        embedding2 = self.hawkes_process_layer3(interval2, embedding2, edge_index2)
        embedding2 = F.relu(embedding2)

        embedding0 = self.linear6(embedding0[data])
        embedding1 = self.linear7(embedding1[data])
        embedding2 = self.linear8(embedding2[data])

        emb0 = self.linear9(central_node_emb0)
        emb1 = self.linear10(central_node_emb1)
        emb2 = self.linear11(central_node_emb2)

        embedding0 = emb0 + embedding0
        embedding1 = emb1 + embedding1
        embedding2 = emb2 + embedding2

        structure_embedding = self.linear12(structure_embedding)
        embedding = self.concat_Layer(embedding0, embedding1, embedding2, structure_embedding)
...
def running(train_loader, test_loader):
    ...
    meta_length2 = get_semantic_network_size(dataset + "/appa_meta_dict.txt")

    # apa
    edge_index0, interval0 = get_edge_index_and_interval(dataset + "/apa_meta_dict.txt",
                                                         dataset + "/apa_graph.edgelist")
    semantic_embedding0 = read_semantic_embedding(
        dataset + "/apa-prone-embedding.txt", meta_length0, meta_length0 +
    ...
    semantic_embedding1 = read_semantic_embedding(
        dataset + "/apcpa-prone-embedding.txt", meta_length1, meta_length1 +
        args.author_num + args.paper_num + args.conf_num)

    # appa
    edge_index2, interval2 = get_edge_index_and_interval(dataset + "/appa_meta_dict.txt",
                                                         dataset + "/appa_graph.edgelist")
    semantic_embedding2 = read_semantic_embedding(
        dataset + "/appa-prone-embedding.txt", meta_length2, meta_length2 +
        args.author_num + args.paper_num + args.conf_num)

    apa_central_node_emb = torch.tensor(semantic_embedding0[-1 * (args.author_num + args.paper_num + args.conf_num):
    ...
    apcpa_central_node_emb = torch.tensor(semantic_embedding1[-1 * (args.author_num + args.paper_num + args.conf_num):
                                                              -1 * (args.paper_num + args.conf_num)]).float().to(device)
    interval1 = torch.tensor(interval1).float().to(device)
    edge_index1 = torch.LongTensor(edge_index1).to(device)

    appa_central_node_emb = torch.tensor(semantic_embedding2[-1 * (args.author_num + args.paper_num + args.conf_num):
                                                             -1 * (args.paper_num + args.conf_num)]).float().to(device)
    interval2 = torch.tensor(interval2).float().to(device)
    edge_index2 = torch.LongTensor(edge_index2).to(device)
    ...
    semantic_embedding2 = torch.tensor(semantic_embedding2).float().to(device)
    ...
    model = Net(meta_length0, meta_length1, meta_length2, args.num_classes).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            out = model(semantic_embedding0=semantic_embedding0, interval0=interval0, edge_index0=edge_index0,
                        ...
                        semantic_embedding2=semantic_embedding2, interval2=interval2, edge_index2=edge_index2,
                        central_node_emb0=apa_central_node_emb[data], central_node_emb1=apcpa_central_node_emb[data],
                        central_node_emb2=appa_central_node_emb[data],
        ...
        for batch, (data_test, target_test) in enumerate(test_loader):
            ...
            _, pred = model(semantic_embedding0=semantic_embedding0, interval0=interval0, edge_index0=edge_index0,
                            ...
                            central_node_emb2=appa_central_node_emb[data_test], data=data_test,
                            structure_embedding=structure_embedding[data_test]).max(dim=1)
