...
class GenerateMetaNetwork:
    """Generate semantic networks based on meta-path and star topology
    ...
    def generate_graph(self):
        ...
        :return author_list: all of authors in network
        :return paper_list: all of papers in network
        :return conf_list: all of conferences in network
        """
        G = nx.Graph()
        G_edgelist = nx.Graph()
        author_list = []
        ...
        with open("data/output/" + self.dataset + "/paper_author.txt", mode="r", encoding="UTF-8") as f:
            for line in f:
                s = line.strip("\n").split(" ")
        ...
        with open("data/output/" + self.dataset + "/paper_paper.txt", mode="r", encoding="UTF-8") as f:
            for line in f:
                s = line.strip("\n").split(" ")
                if "p" + s[0] not in paper_list:
                    paper_list.append("p" + s[0])
                if "p" + s[1] not in paper_list:
                    paper_list.append("p" + s[1])
                G.add_node("p" + s[0], node_type="paper")
                G.add_node("p" + s[1], node_type="paper")
                G.add_edge("p" + s[0], "p" + s[1], time=s[2])
                G_edgelist.add_edge(int(s[0]) - 1 + args.author_num, int(s[1]) - 1 + args.author_num, weight=1)

        with open("data/output/" + self.dataset + "/paper_conf.txt", mode="r", encoding="UTF-8") as f:
            for line in f:
                s = line.strip("\n").split(" ")
                if "p" + s[0] not in paper_list:
                    paper_list.append("p" + s[0])
            ...
            for l in range(len(node_list)):
                ...
                for num in range(args.sample_num):
                    ...
                    if M == "apa":
                        ...
                        time_list = [t1, t1, t2, t2]
                    elif M == "apcpa":
                        time_list = [t1, t1, str((int(t1) + int(t2)) // 2), t2, t2]
